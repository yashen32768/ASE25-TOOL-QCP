...
Definition cast_unsigned (z: Z): Z := z mod modulus.
Definition cast_signed (z: Z): Z :=
  let x := cast_unsigned z in
...
Lemma eqm_mod_eq:
  forall x y, eqm x y -> x mod modulus = y mod modulus.
Proof.
  intros.
  apply (eqmod_mod_eq modulus).
  + apply modulus_pos.
  + apply H.
Qed.
...
Lemma eqm_signed_repr:
  forall z, eqm z (signed (repr z)).
Proof.
  intros.
  eapply eqm_trans; [| apply eqm_sym, eqm_signed_unsigned].
  auto with ints.
Qed.
...
#[export] Hint Rewrite bits_zero bits_mone : ints.
...
#[export] Hint Rewrite bits_and bits_or bits_xor bits_not: ints.
...
#[export] Hint Rewrite bits_shl bits_shru bits_shr: ints.
...
#[export] Hint Rewrite bits_rol bits_ror: ints.
...
#[export] Hint Rewrite bits_zero_ext bits_sign_ext: ints.
