...
#[export] Instance Pred_mL (A: Type) {R: KM.Relation A}: ModalLanguage (Pred_L A) :=
  Build_ModalLanguage (Pred_L A) Semantics.boxp.

#[export] Instance Pred_kmSM (A: Type) {R: KM.Relation A}: @KripkeModalSemantics (Pred_L A) (Pred_minL A) (Pred_mL A) (Build_Model A) (unit_kMD _) tt R (Pred_SM A).
Proof.
  constructor.
  intros; apply Same_set_refl.
Qed.

#[export] Instance Pred_KmGamma (A: Type) {R: KM.Relation A}: SystemK (Pred_L A) (Pred_Gamma A).
Proof.
  constructor.
  + intros x y.
