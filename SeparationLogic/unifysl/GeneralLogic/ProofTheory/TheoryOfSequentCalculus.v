...
Class BasicSequentCalculus_weak (L : Language) (Gamma : Derivable L) := {
  __deduction_weaken: forall Phi Psi x, Included _ Phi Psi -> Phi |--- x -> Psi |--- x;
  __derivable_assum: forall Phi x, Ensembles.In _ Phi x -> Phi |--- x;
}.

Class BasicSequentCalculus_subst1 (L : Language) (Gamma : Derivable L) := {
  __deduction_subst1 : forall Phi x y, Phi |--- x -> Phi;; x |--- y -> Phi |--- y;
}.

Class BasicSequentCalculus_subst (L : Language) (Gamma : Derivable L) := {
  __deduction_subst: forall (Phi Psi: context) y, (forall x, Psi x -> Phi |--- x) -> Union _ Phi Psi |--- y -> Phi |--- y;
}.

Class BasicSequentCalculus_fw (L : Language) (Gamma : Derivable L) := {
  __derivable_finite_witnessed: forall (Phi: context) (y: expr), Phi |--- y -> exists xs, Forall Phi xs /\ (fun x => In x xs) |--- y;
}.

Lemma Subst1FiniteWitness2Subst 
  {L : Language}
  {Gamma : Derivable L}
  ...
  BasicSequentCalculus_subst L Gamma.
Proof. 
  constructor. 
  apply DeductionWeaken_ContextualDerivableFiniteWitnessed_DeductionSubst1_2_DeductionSubst.
  + hnf. apply __deduction_weaken.
  + hnf. apply DeductionWeaken_DerivableFiniteWitnessed_2_ContextualDerivableFiniteWitnessed.
    - hnf. apply __deduction_weaken.
    - hnf. apply __derivable_finite_witnessed.
  + hnf. apply __deduction_subst1.
Qed.
